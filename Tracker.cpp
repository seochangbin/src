...
int SingleTracker::isTargetInsideFrame(int _frame_width, int _frame_height, cv::Mat *mask)
{
	...
	if( mask != nullptr ){
		...
		//cv::Canny(gray, gray, 200, 200, 3);

        cv::GaussianBlur(gray, gray, cv::Size(3,3), 0, 0, cv::BORDER_DEFAULT);
	   	cv::cvtColor(gray , gray, cv::COLOR_BGR2GRAY);
        cv::Laplacian(gray, gray, ddepth, 3, 1, 0, cv::BORDER_DEFAULT);		
...
{
	double max_overlap_thresh = 0.5;
	double dist_thresh = rect.height*rect.width; // Pixels^2 -> adjust properly (maybe a proportion of the img size?) >>1
	std::vector<std::shared_ptr<SingleTracker>> selection;
	std::shared_ptr<SingleTracker> best = NULL;
	double min_distance = (rect.height*rect.width)+5; // Init bigger than threshold +10
	int index;
