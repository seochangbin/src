...
void CallBCrop(int event, int x, int y, int flags, void *scn)
{
	...
	if(event==cv::EVENT_LBUTTONDOWN && !release){
		std::cout << "Left mouse button clicked at lkw(" << x << ", " << y << ")" << std::endl;
		x1 = x;
		y1 = y;
		release = true;
	}
...
void CallBDraw(int event, int x, int y, int flags, void *scn)
{
	...
	if(event==cv::EVENT_LBUTTONDOWN){
		std::cout << "Left mouse button clicked at (" << x << ", " << y << ")" <<sceneRef.vertices.size()<< std::endl;
...
int DrawAreasOfInterest(const cv::String & winname, RegionsOfInterest *scn)
{
	...
	cv::String outstr = "select more than 4 point for  ROI by clicking Mouse ";
	cv::String outstr1 ="Press 'Esc' to finish or use default ROI";
	...
	while(!finished){
		cv::putText(sceneRef.aux, outstr, cv::Point2f(100, 100), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0, 0, 255));
		cv::putText(sceneRef.aux, outstr1, cv::Point2f(100, 150), cv::FONT_HERSHEY_TRIPLEX, 0.5, cv::Scalar(0, 0, 255));
		cv::imshow(winname, sceneRef.aux);
		switch (cv::waitKey(1)) {
			case 'S':
				if(can_finish) {
