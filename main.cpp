...
std::string return_current_date(){
    auto now = std::chrono::system_clock::now();
    auto in_time_t = std::chrono::system_clock::to_time_t(now);
    std::stringstream ss1;
    ss1 << std::put_time(std::localtime(&in_time_t), "%m-%d");
    // ss << std::put_time(std::localtime(&in_time_t), "%Y-%m-%d_%X");
    return ss1.str();
}

std::string return_current_time(){
    auto now = std::chrono::system_clock::now();
    auto in_time_t = std::chrono::system_clock::to_time_t(now);
    std::stringstream ss2;
    ss2 << std::put_time(std::localtime(&in_time_t), "%X");
    // ss << std::put_time(std::localtime(&in_time_t), "%Y-%m-%d_%X");
    return ss2.str();
}
...
bool DoesROIOverlap( cv::Rect boundingbox,std::vector<cv::Point> contour, std::string &res) {
   ...
   if (boundingbox.area()< 4000)
   {
    ...
    std::cout<<"\nx:"<<boundingbox.x<<" y:"<<boundingbox.y << " w:"<<boundingbox.width<<" h:"<<boundingbox.height<<endl;
...
int main(int argc, char *argv[]) {
   ...
   // close(fd);
	...
	if (!FLAGS_show_selection){
        ...
        if(scene.vertices.size() >4 ){
           ...
           mp.vertices.push_back(Point(164, 465));
    ...
    //if(FLAGS_o){
        /*/////////////////////////////////////////////////////////////////////////devbin 수정
        string day = "./res/" + return_current_date() + ".avi" ;
        VideoWriter outputVideo;
        outputVideo.open(day, VideoWriter::fourcc('X', 'V', 'I', 'D'),30, cv::Size(320,270), true);
   ...
   // }
        ...
        int numSyncFrames = 0;
        int totalFrames = 0;
        bool in_Detected= false;
        bool in_Empty = false;
        int totalWait=0;
        int totalDetCar=0;
        std::string Det_in = "Empty";
        ...
        do {
            ...
            if (((!pipeS3toS4Fifo.empty() && !pipeS1toS4Fifo.empty()) &&  vp_enabled) 
                ...
                if(vp2_enabled){
                 ...
                 //       <<" D:" << totalDetected
                  //      <<" W:" << totalWait
                  //      <<" FR:" << fps;
                    //cv::putText(outputFrame_clean, out.str(), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 0, 0));
                    cv::putText(config_image, out.str(), cv::Point2f(10, 10), cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 0, 0));
                       */
                   ...
                   // cv::putText(outputFrame_clean, std::to_string(totalFrames), cv::Point2f(20, 20), cv::FONT_HERSHEY_TRIPLEX, 1,cv::Scalar(0, 255, 0));
                    ...
                    if(wait_flag!=0){
                        ...
                        //fn.str("");
                        in_Detected = true;
                     if(in_Detected == true && in_Empty == false){//devbin 차량 detected
                            totalDetCar += 1;
                            in_Detected = false;
                            in_Empty = true;
                        }
			...
			if (FLAGS_collision) {
               ...
               // int ddepth = CV_8U;
                ...
                if (!FLAGS_no_show) {
                    Mat config_image(30, 320, CV_8UC3, Scalar(255, 255, 255));
                   ...
                   // cv::moveWindow(winname, 10, 10);
                    ...
                    out.str("");
                    out <<return_current_time_and_date()
                        <<" Det: " << totalDetCar << " Det: " << Det_in;
                        

                    //cv::putText(outputFrame_clean, out.str(), cv::Point2f(10, 50), cv::FONT_HERSHEY_SIMPLEX, 1,cv::Scalar(255, 0, 0));
                    cv::putText(config_image, out.str(), cv::Point2f(10, 20), cv::FONT_HERSHEY_SIMPLEX, 0.5,cv::Scalar(0, 0, 0));
                       
                  //  cv::GaussianBlur(outputFrame_clean, outputFrame_clean, Size(3,3), 0, 0, cv::BORDER_DEFAULT);
	              //  cv::cvtColor(outputFrame_clean , outputFrame_clean, cv::COLOR_BGR2GRAY);
                  //  cv::Laplacian(outputFrame_clean, outputFrame_clean, ddepth, 3, 1, 0, cv::BORDER_DEFAULT);
                    
                    //cv::Canny(outputFrame_clean, outputFrame_clean, 200, 200, 3);

                
                    cv::resize(outputFrame_clean, outputFrame_clean, Size(outputFrame_clean.cols/2,outputFrame_clean.rows/2), 0, 0);
                    //cv::resize(config_image, outputFrame_clean, Size(len, 20), 0, 0);
                    cv::vconcat(config_image, outputFrame_clean, outputFrame_clean);
                    ...
                    //waitKey(500);
                    /* ////////////////////////////////////////////devBin 수정
                    if(return_current_time() == "00:00:00"){
                        totalDetCar = 0;
                        string day = "./res/" + return_current_date() + ".avi" ;
                        outputVideo.open(day, VideoWriter::fourcc('X', 'V', 'I', 'D'),30, cv::Size(320,270), true);
                    }
                    ...
                    //string  fns =  "./res/"+return_current_time_and_date()+".jpeg"; 
                    //cv::imwrite(fns, outputFrame_clean);
